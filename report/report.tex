...
the location of the school (country). It is also possible but less likely that
...
sum of weights, sample size, and respondents for each class (country)
(Section \autoref{sec:weights} \hyperref[sec:weights]{Weighting Classes} of
the Appendix).
...
$$
$c$ being the country. The weight factor is then the recipricol of $\hat{\phi}_c$, $\frac{1}{\hat{\phi}_c}$.
